...
void GUI::PrintMessage(string msg, int n) const	//Prints a message on status bar
{
	...
	if (n < 4) {
		pWind->DrawString(10, WindHeight - (int)(StatusBarHeight - (45 + 20 * n)), msg);
	}
	else if (n >= 4 && n < 8) {
		pWind->DrawString((WindWidth/4)+10, WindHeight - (int)(StatusBarHeight - (45 + 20 * (n-4))), msg);
	}
	else if (n >= 8 && n < 12) {
		pWind->DrawString((WindWidth / 2) + 10, WindHeight - (int)(StatusBarHeight - (45 + 20 * (n - 8))), msg);
	}
	else if (n >= 12 && n < 16) {
		pWind->DrawString((3*WindWidth / 4) + 10, WindHeight - (int)(StatusBarHeight - (45 + 20 * (n - 12))), msg);
	}
}
...
//////////////////////////////////////////////////////////////////////////////////////////
void GUI::PrintStats(int ts, int cn, int wo) {
	ClearStatusBar();
	pWind->SetPen(DARKRED);
	pWind->SetFont(18, BOLD, BY_NAME, "Arial");
	pWind->DrawString(10, WindHeight - (int)(StatusBarHeight-5), "Timestamp: " + to_string(ts));
	pWind->DrawString(10, WindHeight - (int)(StatusBarHeight-25), "Number of available cooks" + to_string(cn));
	pWind->DrawString(10, WindHeight - (int)(StatusBarHeight-45), "Number of waiting orders: " + to_string(wo));
}
...
void GUI::AddToDrawingList(Order* pOrd)
{
	...
	case SRV:
		...
		pDitem->ST = pOrd->getServTime();
		break;
	case DONE:
		reg = DONE_REG;	//region of waiting orders
		pDitem->FT = pOrd->getServDur() + pOrd->getServTime();
		break;
	}
	...
	for (int i = cookcount; i <= DrawingItemsCount; i++) {
		for (int j = i + 1; j <= DrawingItemsCount; j++) {
			if (DrawingList[i]->region == ORD_REG && DrawingList[j]->region == ORD_REG && DrawingList[j]->ID < DrawingList[i]->ID ) {
				DrawingItem* ptemp = new DrawingItem;
				ptemp = DrawingList[i];
				DrawingList[i] = DrawingList[j];
				DrawingList[j] = ptemp;
			}
			else if (DrawingList[i]->region == SRV_REG && DrawingList[j]->region == SRV_REG && DrawingList[j]->ST< DrawingList[i]->ST) {
				DrawingItem* ptemp = new DrawingItem;
				ptemp = DrawingList[i];
				DrawingList[i] = DrawingList[j];
				DrawingList[j] = ptemp;
			}
			else if (DrawingList[i]->region == DONE_REG && DrawingList[j]->region == DONE_REG && DrawingList[j]->FT < DrawingList[i]->FT) {
				DrawingItem* ptemp = new DrawingItem;
				ptemp = DrawingList[i];
				DrawingList[i] = DrawingList[j];
				DrawingList[j] = ptemp;
...
void GUI::AddToDrawingList(Cook* pC)
{
	...
	if (pC->getBS()!= -858993460) {
		pDitem->BT = pC->getBS() + pC->getBD();
	}
	else {
		pDitem->BT = 0;
	}
	cookcount++;
	DrawingList[DrawingItemsCount++]=pDitem;
	for (int i = 0; i <= DrawingItemsCount-1; i++) {
		for (int j = i + 1; j <= DrawingItemsCount-1; j++) {
			if (DrawingList[i]->region == COOK_REG && DrawingList[j]->region == COOK_REG && DrawingList[j]->BT < DrawingList[i]->BT) {
				DrawingItem* ptemp = new DrawingItem;
				ptemp = DrawingList[i];
				DrawingList[i] = DrawingList[j];
				DrawingList[j] = ptemp;
...
PROG_MODE	GUI::getGUIMode() const
{
	...
	do
	{
		PrintMessage("Please select GUI mode: (1)Interactive, (2)StepByStep, (3)Silent, (4)simplesim... ");
		string S = GetString();
...
void GUI::retMouseClick(int &x, int &y) {
